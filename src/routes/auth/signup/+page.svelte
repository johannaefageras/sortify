...
<main class="auth-page">
	<section class="auth-card">
		<div class="auth-top">
			<div>
				<p class="ui-label">Skapa konto</p>
				<h1 class="auth-title">Registrera dig</h1>
			</div>
			<a href={`/auth/login?next=${encodeURIComponent(data.next)}`} class="auth-alt-link">Logga in</a>
		</div>

		<p class="auth-hint">Skapa ditt konto med Google, lösenord eller en magisk länk.</p>
		...
		{#if form?.success}
			<div class="auth-success">
				<p>
					Kolla <strong>{form.email}</strong> efter en bekräftelse eller magisk länk för att fortsätta.
				</p>
			</div>
		{/if}
		...
		<form method="POST" action="?/password" class="auth-form">
			...
			<button class="btn btn-primary auth-submit" type="submit">Skapa konto</button>
		</form>
